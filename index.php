			...
			if(trim($cmdArr[0] == '违章')){
                if(count($cmdArr)==3){
					...
					if((is_array($checkResultArray)==1) && (count($checkResultArray)==4) && ($checkResultArray[0]==true)){
						...
						$contentStr = "命令格式错误，正确的格式为：违章+NAAAAA+12345";
                    ...
                    if($flag == 1){
						...
						//没有绑定信息
						$contentStr = "当前用户还没有绑定车牌，请先绑定信息，可输入“违章+Naaaaa+12345”指令将账号与车牌进行绑定";
                        $resultStr = $this->transmitText($object, $contentStr);
                        }
                    }elseif($flag > 1){
            	...
            	$contentStr = "暂时不支持[".$keyword."]指令。\n正确的指令是“请求+参数”，如违章查询输入：违章+N1234B+12345，天气查询：天气+怀化。如需详细帮助请输入？查询";
				$resultStr = $this->transmitText($object, $contentStr);
                //return $resultStr;
            }
		}
		...
		if(count($title)==4){
			...
			if($textCount==0 && $valueCount==0){
                ...
                */
            	...
            	$instoreflag = $this->checkDatatmp($valueArray, $plateNumber, $engineNumber);		//将违章信息存入数据库
